...
\item Algorithms $A$ and $M$ are constructed in Sections \ref{subsec:appxalgo} and \ref{sec:minalgo}, which solve problems \eqref{appxprob} and \eqref{optprob}, respectively.  These algorithms are based on linear splines.  Guarantees of success are provided by Theorems  \ref{thm:algAworks} and \ref{thm:algMworks}.
...
& \le (b-a)\sqrt{\frac{\fC(0) \norm[\frac 12]{f''}}{8\abstol}} +1 \label{costbdinf}  \qquad \text{by \eqref{halflessinf}}.
\end{align}
...
For any real $\gamma$, it follows that $\gamma f_3(x_i)=0$ for $i=0, \ldots, n+1$.  Since $0$ and $\pm \gamma f_3$ share the same values at the data sites, then they must share the same approximation: $A^*(\pm \gamma f_3,\abstol) = A^*(0,\abstol)$.  This implies  that $\gamma$ must be no greater than $\abstol$
	\begin{align*}
	...
	\end{cases}
	\end{align*}
The top inequality cannot be satisfied unless $\sigma = \norm[\frac 12]{\gamma f_3''}$ is small enough.  Solving the bottom inequality for $n$ in terms of $\sigma = (b-a)^2 \norm{\gamma f_3''}/[16(n+1)^2]$ establishes  \eqref{lowbdWinf}.

	
	
	
We firstly try to prove the lower complexity bound for the function in cone $\cc$ and $\norm[\frac12]{f''} \le \sigma$.
We choose a parabola satisfied the above conditions
\begin{equation}
\label{assumpfzero}
f_0(x) =\frac{x^2}{2}, \qquad \norm[\frac12]{f_0''} \le \sigma  \qquad x \in [a,b].
\end{equation}
Thus $A^*$ successfully approximate $f_0$.

Suppose that one has the sample points
$(\xi_i)_{i=1}^n$ for arbitrary $\xi_i$, where $a=\xi_0 \le \xi_1 < \cdots < \xi_n \le \xi_{n+1} = b$.  There must be some $j=0, \ldots, n$ such that $$\xi_{j+1} - \xi_j \ge \frac{b-a}{n+1}.$$  For any fixed $\abstol>0$, let $\left(\xi_i\right)_{i=1}^n$ be the design used by $A^*$ for the function
$f_0$.  Thus, we obtain $\cost(A^*,f_0,\abstol) =n.$

Consider the function $f_{3}$ is defined as a bump having piecewise constant second derivative on $[\xi_j, \xi_{j+1}]$ and zero elsewhere, where
\begin{equation}\label{deltan}
a\le c-2\delta < c+2\delta \le b \qquad 4\delta \ge \frac{b-a}{n+1}.
\end{equation}
Next, we introduce two fooling functions, $f_{\pm}=c_1(f_{0} \pm c_2 f_3)$, in terms of $f_0$ defined as \eqref{assumpfzero} and $f_3$ defined before, where
\begin{equation}
\label{defofc1c2}
c_1=\frac{\norm[\frac 12]{f_0''}}{(b-a)^2}, \qquad c_2 =\frac{\fC(0)-1}{\fC(0)+1}\delta^2.
\end{equation}

Therefore, for both fooling functions we have
\[
\norm[\infty]{f''_{\pm}} = c_1\left(1+\frac{c_2}{\delta^2}\right), \qquad m(f''_{\pm},\alpha,\beta) \ge c_1\left(1-\frac{c_2}{\delta^2}\right), \
 [\alpha,\beta] \subset [a,b].\]

Moveover, we have
\begin{align*}
\max\abs{f_{\pm}''(x)} \le \norm[\infty]{f''_{\pm}} & = c_1\left(1+\frac{c_2}{\delta^2}\right) \\
&  = \fC(0)c_1\left(1-\frac{c_2}{\delta^2}\right) \qquad \text{by } \eqref{defofc1c2}\\
& \le \fC(0) m(f''_{\pm},\alpha,\beta) \\
& \le \begin{cases}
\fC(h_-) m(f''_{\pm},x-h_-,x) & a \le x-h_-\le x \le h,\\
\fC(h_+) m(f''_{\pm},x,x+h_+) & a \le x\le x+h_+ \le h.
\end{cases}
\end{align*}

Thus,
\[ \max\abs{f_{\pm}''(x)}  \le \tB(f_{\pm},x,h_-,h_+) \qquad \forall  \max(h_{\pm}) \in ]0, \fh[. \] We obtain $f_{\pm} \in \cc.$
Hence, algorithm $A^*$ also works for $f_{\pm}$.
We note that the data used by algorithm $A^*$ for both fooling functions is the same, so $A(f_{\pm},\abstol)=A(c_1f_0,\abstol)$.  Consequently,
\begin{align*}
\abstol & \ge \max(\norm[\infty]{f_+-A(f_+)},\norm[\infty]{f_--A(f_-)}) \\
...
& = c_1c_2= \frac{\norm[\frac 12]{f_0''}}{(b-a)^2}\cdot\frac{(\fC(0)-1)\delta^2}{\fC(0)+1} \qquad\text{ by } \eqref{defofc1c2}\\
& \ge \frac{\norm[\frac 12]{f_0''}}{(b-a)^2}\cdot\frac{(\fC(0)-1)(b-a)^2}{16(\fC(0)+1)(n+1)^2} \qquad \text{ by } \eqref{deltan}\\
& =\frac{\fC(0)-1}{16(\fC(0)+1)(n+1)^2}\norm[\frac 12]{f_0''}
\end{align*}
Thus, we have
\[ \cost(A^*,f_0,\abstol) =n \ge \sqrt{\frac{(\fC(0)-1)\norm[\frac 12]{f_0''}}{16(\fC(0)+1)\abstol}} -1.\]
Hence,
\[\sup_{f \in \cc : \norm[\frac12]{f''} \le\sigma } \cost(A^*,f,\abstol) \ge \sqrt{\frac{(\fC(0)-1)\sigma}{16(\fC(0)+1)\abstol}} -1. \]

Similarly, we can let $c_1=\norm{f_0''}$, then we obtain
\[ \cost(A^*,f_0,\abstol) =n \ge \sqrt{\frac{(\fC(0)-1)\norm{f_0''}}{16(\fC(0)+1)\abstol}} -1.\]
Hence,
\[\sup_{f \in \cc : \norm{f''} \le\sigma } \cost(A^*,f,\abstol) \ge \sqrt{\frac{(\fC(0)-1)\sigma(b-a)^2}{16(\fC(0)+1)\abstol}} -1. \]

We proved \eqref{lowbdC}. Next we consider algorithms works for the functions not inside the cone.
\end{proof}
