...
\begin{multline} \label{conedef}
 \cc :=   \Bigl \{
 f  \in    \cw^{2,\infty}[a,b]:   \max\abs{f''(x)}  \le M(f,x,h_\pm)  \text{ for all } x \in [a,b],
...
$n_{\text{hi}} \ge n_{\lo}$ and let  $n_0=n_{\ninit}-1$ where
\begin{equation}
\label{nodefinition}
...
\end{equation}
Define the
following $n_0+1$ points and index set of intervals for which the error tolerance
is not yet satisfied: $$x_i=a+\frac{i}{n_0}(b-a), \ i=0,\ldots,n_0, \qquad
\mathcal{I} = \{1,2,\ldots,n_0-1,n_0\},$$ and choose $\fC$. Compute $f[x_{i-1},
x_{i}, x_{i+1}]$ for $i = \{1,2,\ldots,n_0-1\}$ as in \eqref{divdiff}. Then do the
following:
...
\]
Then update $\mathcal{I}$ is $\mathcal{I}_+ \cup \mathcal{I}_-$.
If $\mathcal{I} = \emptyset$, return the linear spline $A(f,\varepsilon) = S(f, \{x_i\}_{i=0}^{n_0})$ and terminate the algorithm.
...
Update $n_0$, $\{x_i\}_{i=0}^{n_0}$. Update $\mathcal{I}$ to include the indices of all intervals whose indices were included in the old $\mathcal{I}$ accordingly. Return to Step~\ref{stage1}.
\end{enumerate}
\end{algoA}
