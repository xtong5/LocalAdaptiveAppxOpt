...
The goal is to solve the problems of function approximation and optimization by locally adaptive algorithms. For some suitable set, $\cc$, of continuous,
...
\end{gather}
Here, $\norm{\cdot}$ denotes the sup-norm.  The algorithms $A$ and $M$ depend only on function values. They choose the data sites in $[a,b]$ adaptively, with each new site depending on the function data already obtained.  The algorithms sample more densely where needed, i.e., they are locally adaptive.  These
algorithms also automatically determine when to stop sampling the input function
and return the correct answers.

Adaptive algorithms relieve the user of having to specify the number of samples required or some equivalent information.  The user only needs to be satisfied with the parameters that define $\cc$ and so determine the robustness of the algorithms.

We accomplish the following:
\begin{itemize}

\item The set $\cc$ is defined in Section \ref{sec:conedef} to be a cone containing functions whose second derivatives have finite sup-norms.  Moreover, functions in $\cc$ are not too spiky.  Drawing on Newton's divided differences in Section \ref{sec:ndd}, we construct a data-based upper bound on $\norm[{[\alpha,\beta]}]{f''}$ in \eqref{normbd} and ultimately a data-based upper bound on the error of the linear spline in \eqref{appxerrbdb}.
...
\item Upper bounds on the computational costs of these algorithms  are derived in Sections \ref{subsec:appxcost} and \ref{subsec:optcost}.  The upper bounds in Theorems \ref{thm:cost}  and  ?? are roughly proportional to $\Bignorm[1]{\sqrt{\abs{f''}}}/\sqrt{\abstol}$ (see \eqref{costbdapprox}).  This asymptotic behavior can be much less than the $\sqrt{\norm{f''}/\abstol}$ that one might expect (see Proposition \ref{equivnormprop}).
...
\begin{multline*}
\widehat{\mathcal{I}}_{\pm k} = \left\{i \in \{1,2,\ldots,n_0\}: \frac{(x_j - x_{j-1})^2B_{j,\pm}(f,\cp)}{8}  > \abstol,\right.\\
...
\begin{prop} \label{equivnormprop} The quantities $\norm{f''}$ and $\Bignorm[1]{\sqrt{\abs{f''}}}^2$ bound each other as follows:
	\begin{subequations}
		\begin{gather}
		\max_{0 \le h \le \fh} \frac{h^2}{\fC(h)} \norm{f''} \le \Bignorm[1]{\sqrt{\abs{f''}}}^2 \qquad \forall f \in \cc \\
		 \Bignorm[1]{\sqrt{\abs{f''}}}^2  \le (b-a)^2 \norm{f''} \qquad \forall f \in \cw^{2,\infty}.
		\end{gather}
	\end{subequations}
\end{prop}
\begin{proof}
...
Although $\norm{f''}$ and $\Bignorm[1]{\sqrt{\abs{f''}}}^2$ are equivalent for $f \in \cc$, that is not the case for $f \in \cw^{2,\infty}$.  The hump function $f_3$ defined above satisfies
\[
\norm{f_3''} = \delta^{-2}, \qquad \Bignorm[1]{\sqrt{\abs{f''}}}^2 = 16.
\]
By making $\delta$ small enough, we may make $\norm{f_3''}/\Bignorm[1]{\sqrt{\abs{f''}}}^2$ arbitrarily large.  However, since $f_3 \notin \cc$ for $\delta < 2\fh$, this does not violate Proposition  \ref{equivnormprop}.
