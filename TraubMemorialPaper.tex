...
\begin{itemize}
\item Algorithms $A$ and $M$ are constructed in Sections \ref{subsec:appxalgo} and \ref{sec:minalgo}, which solve problems \eqref{appxprob} and \eqref{optprob}.  See Theorems  \ref{thm:algAworks} and \ref{thm:algMworks}.
...
Linear splines form the basis for our adaptive algorithms $A$ and $M$. The best linear spline error possible occurs for the Sobolev space of functions whose second derivatives have finite $L^{\infty}$-norm:
\[
...
\subsection{Computational Cost} \label{subsec:appxcost}
