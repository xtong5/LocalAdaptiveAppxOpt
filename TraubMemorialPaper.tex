...
\subsection{The Cone, $\cc$}
The set of functions $\cc$ for which our algorithms are successful will be a cone, i.e., $f \in \cc \implies af \in \cc $ for all real $a$.  Cones of functions have are key to the adpative algorithms and their analysis  in \cite{HicEtal14b},  \cite{Din15a} and \cite{Ton14a}.  The cone chosen here is somewhat different than those cones, but similar to the cone  used by \cite{HicRazYun15a}. 

Our adaptive algorithms will be based on linear splines.  The normed vector space of functions with essentially the weakest smoothness assumptions required to guarantee the best error possible for the linear spline is the following Sobolev space of functions whose second derivatives have finite $L^{\infty}$-norm:
\[
\cw^{2,\infty}[a,b] : = \Bigl \{f \in C^1[a,b] : \norm{f''} : = \norm[{[a,b]}]{f''} : = \sup_{a \le x \le b} \abs{f''(x)} <  \infty \Bigr\}.
\]
The $L^{\infty}$-norm appears in the error bound for linear splines, and our algorithm must construct an upper bound in terms of function values.

Although we cannot obtain values of $f''(x)$ for a specific $x$ of our choosing based only on values of $f$, we can use values of $f$ to determine $f''(x)$ \emph{for some unknown $x$} in a specified interval.  This is done via Newton divided differences.  For any $\{x_1, x_2, x_3\}$ satisfying $a \le x_1 \le x_2 \le x_3 \le b$ define
\begin{equation} \label{minfppdef}
m(f,[x_1, x_3]) = \min \Bigl\{ \abs{z}  : \inf_{x_1 \le x \le x_3} f''(x) \le z \le \sup_{x_1 \le x \le x_3} f''(x) \Bigr\}.
\end{equation}
Here the infimum and supremum exclude points where $f''$ is undefined.  Note that $m(f,[x_1, x_3])$ is either $ \inf_{x_1 \le x \le x_3} \abs{f''(x)}$ or $0$ depending on whether $f''$ takes on one sign or both signs on the interval  $[x_1, x_3]$.  Note that for any intervals $\ci, \cj \subseteq [a,b]$ it follows from the definition that
\begin{equation} \label{mdec}
m(f,\ci) \ge m(f,\cj) \qquad \forall \ci \subseteq \cj.
\end{equation}
...
\cc : = \bigl \{ f \in \cw^{2,\infty}[a,b] : \text{for all }x \in [a,b], \ x_\pm = x \pm h_\pm, \ h = \max(h_{\pm}) \in [0, \fh], \\  \abs{f''(x)}
...
\end{equation*}
Applying the fact that $\fC$ is non-decreasing and property \eqref{mdec} yields the following upper bound for the norm of $f''$ in terms of function values:
...
Next consider the cases of the sub-intervals on the left border of the interval.  For $i \in \{1,2\}$ it follows by a similar argument that 
\begin{align}
\nonumber
...
\item[Stage 3.\ Split intervals in $\mathcal{I}$ and adjacent intervals to $i, i \in \mathcal{I}$.]\label{stage3} %Suppose
