...
Next, we investigate the computational cost of our locally adaptive algorithm. Let $n_0= n_{\ninit} - 1$ denote the initial number of intervals, and
 $h_0=(b-a)/{n_0}$ be the initial width of the subintervals. To faciliate our derivation of the compuational cost of $A$, we  introduce the new notation $\Ixl$, which is the unique interval contains $x$ with
 the width $2^{-l}h_0$.
\[\Ixl :=\left[a+(j-1)2^{-l}h_0,a+j \ 2^{-l}h_0\right), \qquad j=\left\lceil\frac{(x-a)2^l}{h_0}\right\rceil, \ l \in \mathbb{N}_0.\]
Let
...
Let $\bar{I}_{x,l}$ be a similar interval with generally five times the width:
...
Let $A(f,\varepsilon)$ be the adaptive linear spline defined by Algorithm \textnormal{\texttt{funappx\_g}}, and let $n_0 = n_{\ninit} - 1$, and $\varepsilon$ be the inputs and parameters described there. Let $\cc$ be the cone of functions defined in~\eqref{conedef}.
