...
Compute $B_{i,\pm}(f,\cp)$ as in~\eqref{bpdef}, where $i \in \mathcal{I}$.
Let
\[
...
We now show that $\ell(x) \le L(x)$.  At each iteration of Algorithm $A$, $x$ lies in $\Ixl$ for some $l$, and by the time algorithm $A$ terminates, all values of $l = 0, \ldots, \ell(x)$ are realized.  If $\ell(x) > L(x)$, then at some iteration $I_{x,L(x)}$ must be split in Step~\ref{stage2} of $A$.  Let $\cp = \{x_j\}_{j=0}^n$ denote the partition of $[a,b]$ when $I_{x,L(x)}$ is to be split, and then identify the closure of $I_{x,L(x)}$ as $[x_{i-1},x_i]$ for some $i \in \{1, \ldots, n\}$, where $x_i-x_{i-1}=2^{-L(x)}h_0$.
...
B_{j,s}(f,\cp)(x_j-x_{j-1})^2 \le  \fC\left(3\cdot2^{-L(x)}h_0\right) (2^{-L(x)}h_0)^2  \norm[\bar{I}_{x,L(x)}]{f''}.
\end{equation}
...
2^{-L(x)}h_0 = x_{i} - x_{i-1} \ge x_{j+k} - x_{j+k-1} \qquad \text{for all } k \in \{0,1,2\}.
\end{equation}
In particular, $(x_j-x_{j-1})^2 \le (2^{-L(x)}h_0)^2$, which is part of the proof of \eqref{Blenorm}.  Also, note that
  \begin{align*}
  B_{j,+}(f,\cp)
...
\chL(x) = \min \biggl\{ l \in \mathbb{N}_0 :  \frac{(2^{-l}h_0)^2}{8} \{[\fC\left(3\cdot2^{-l}h_0\right) + 1]\norm[\bar{I}_{x,l}]{f''} + \norm[\bar{I}_{x^*,l}]{f''}\} \\
...
\begin{proof}
	Using the same argument as for Theorem \ref{thm:cost}, we only need to show that $\tell(x) \le \tL(x)$ for all $x \in [a,b[$.  Since $\minfii - \widehat{M} \ge 0$, the index sets $\ci_{\pm}$ and $\widehat{\ci}_{\pm}$ in Algorithm $M$ are subsets of the corresponding index sets in Algorithm $A$.  Thus $\tell(x) \le L(x)$ by the same argument as used to prove Theorem \ref{thm:cost}.  Thus, we only need to show that $\tell(x) \le \chL(x)$.
	
	For a fixed $x$, if $L(x) \ge \tell(x) > \chL(x)$, then at some iteration $I_{x,\chL(x)}$ must be split in Step~\ref{stagemin2} of $M$.  Let $\cp = \{x_j\}_{j=0}^n$ denote the partition of $[a,b]$ when $I_{x,\chL(x)}$ is to be split, and then identify the closure of $I_{x,\chL(x)}$ as $[x_{i-1},x_i]$ for some $i$, where $x_i-x_{i-1}=2^{-\chL(x)}h_0$. We show that such a scenario is impossible, thus contradicting the assumption that $\tell(x) > \chL(x)$.
...
B_{j,s}(f,\cp)(x_j-x_{j-1})^2 \le  \fC\left(3\cdot2^{-\chL(x)}h_0\right) (2^{-\chL(x)}h_0)^2  \norm[\bar{I}_{x,\chL(x)}]{f''}.
\end{equation}

Next, we bound $\min\bigl(f(x_{j-1}),f(x_j) \bigr)$, which appears in the definition of $\ci_{\pm}$ in Algorithm $M$.  Consider the case of $s=+$, so $j = i  - k \in \ci_+$ for some $k \in \{0,1,2\}$.  As was argued earlier, because $i \in \widetilde{\ci}$ it follows that $[x_{j-1},x_{j+2}] \in \bar{I}_{x,\tL(x)}$.  Then a Taylor expansion about $x \in [x_{i-1},x_i [$, establishes that 
\begin{multline} \label{minfjjm1bd}
\min\bigl(f(x_{j-1}),f(x_j) \bigr) \\
\ge f(x) - \abs{f'(x)}3 \cdot 2^{-\chL(x)}h_0 - \frac{\norm[\bar{I}_{x,\chL(x)}]{f''}\bigl(3 \cdot 2^{-\chL(x)}h_0\bigr)^2}{6}
\end{multline}
The same bound also holds for the $s = -$ case.

Finally, we bound $ \widehat{M}$.  Let $[x_{i^*-1}, x^*_i[$ be the interval in $\cp$ containing the location of the minimum, $x^*$.  Note that by \eqref{appxerrbdapc} it follows that
\[
...
\end{align*}
There are two possibilities regarding $i^*$.  If $i^* \in \ci$, then $x_i -  x_{i-1} \ge x_{i^*} -  x_{i^*-1}$, since $i \in \widetilde{\ci}$ and so
\begin{align*}
\hM  & \le f(x^*) + \frac{(x_{i^*} - x_{i^*-1})^2\norm[{[x_{i^*-1}, x^*_i]}]{f''}}{8} \\
& \le  f(x^*) + \frac{(2^{-\tL(x)} h_0)^2\norm[{\bar{I}_{x^*,\tL(x)}}]{f''}}{8}.
\end{align*}
Otherwise, if $i^* \notin \ci$, then by the definition of $\ci$ in Step 1 of Algorithm $M$,
\begin{align*}
\hM  & \le \min(f(x_{i^*-1}),f(x_{i^*})) - \frac{(x^*_i - x_{i^*-1})^2 \max \bigl(B_{i^*,\pm}(f,\cp) \bigr)}{8} + \abstol   \\
& \le \min(f(x_{i^*-1}),f(x_{i^*})) - \frac{(x^*_i - x_{i^*-1})^2 \norm[{[x_{i^*-1}, x^*_i]}]{f''}}{8} + \abstol   \\ 
& \le  f(x^*) +\abstol.
\end{align*}
Thus, we have
\begin{equation} \label{Mhatbd}
\hM \le f(x^*) + \max\biggl(\abstol, \frac{(2^{-\tL(x)} h_0)^2\norm[{I_{x^*,\tL(x^*)}}]{f''}}{8}\biggr).
\end{equation}

Combining the three inequalities \eqref{Blenormt}, \eqref{minfjjm1bd}, and \eqref{Mhatbd} yields the inequality that allows us to contradict the assumption that $\tell(x) > \tL(x)$:
\begin{multline}
B_{j,s}(f,\cp)(x_j-x_{j-1})^2 + \hM - \min(f(x_{i^*-1}),f(x_{i^*})) \\
\le  \fC\left(3\cdot2^{-\tL(x)}h_0\right) (2^{-L(x)}h_0)^2  \norm[\bar{I}_{x,\tL(x)}]{f''} \\
+ f(x^*) + \max\biggl(\abstol, \frac{(2^{-\tL(x)} h_0)^2\norm[{I_{x^*,\tL(x^*)}}]{f''}}{8}\biggr) \\
+   f(x) -  \abs{f'(x)}3 \cdot 2^{-\tL(x)}h_0 - \frac{\norm[\bar{I}_{x,\tL(x)}]{f''}\bigl(3 \cdot 2^{-\tL(x)}h_0\bigr)^2}{6} .
\end{multline}
...
Note that by \eqref{appxerrbdapc} it follows that
\[
f(t) \ge \min(f(x_{j-1}),f(x_{j})) - \frac{(x_{j} - x_{j-1})^2\norm[{[x_{j-1}, x_j]}]{f''}}{8}, \quad \forall t \in [x_{j-1}, x_j]
\]
