...
\norm[\frac 12]{f''} \ge \biggl\{ \int_{a}^{b} \sqrt{\abs{f''(x)}} \, \dif x \biggr\}^2 \ge  (b - a)^2 m(f,a,b)  \ge  (b - a)^2 \norm{f''}.
\end{equation}
\end{proof}
...
\begin{theorem}
	Consider problem \eqref{appxprob}, and let $A^*$ be any deterministic algorithm that successfully solves this problem.  We consider two cases.
	
	\begin{enumerate}
		\renewcommand{\labelenumi}{\roman{enumi}.}
		\item  The problem is solved for $f \in \cc$.  Then for any $\sigma >0$ and $\abstol >0$, there exists an $f \in \cc$ with $\norm[\frac12]{f''} = \sigma$ such that 
		\[
		\cost(A^*,f,\abstol) \ge .
		\] 
		
		\item The problem is solved for $f \in \cw^{2,\infty}$.  Then for any $\sigma >0$ and $\abstol >0$, there exists an $f \in \cw^{2,\infty}$ with $\norm{f''} = \sigma$ such that 
		\[
		\cost(A^*,f,\abstol) \ge .
		\] 
		For any $\sigma > \abstol >0$ and any $n \in \naturals$, there exists an $f \in  \cw^{2,\infty}$ with $\norm[\frac12]{f''} = \sigma$ such that 
    	\[
				\cost(A^*,f,\abstol) \ge n.
		\] 
