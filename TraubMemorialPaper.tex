...
\begin{abstract}
Existing adaptive algorithms for function approximation and optimization lack theoretical guarantees.  Our new locally adaptive algorithms are guaranteed to provide answers to these two problems that satisfy a user-specified absolute error tolerance for a cone of non-spiky functions in $\cw^{2,\infty}$.  These algorithms automatically determine where to sample the function---sampling more densely where needed.  The computational cost to approximate a function with $L^{\infty}$ error no greater than $\abstol$ is $\Order\Bigl(\sqrt{\norm[\frac12]{f''}/\abstol}\Bigr)$, which is the same order as that of the best function approximation algorithm for this cone of functions. The computational cost of the new optimization algorithm is asymptotically no worse than that for function approximation and can be much smaller if the function is significantly larger than its minimum value over much of its domain.  Our new algorithms have been implemented in a software library.  Numerical examples are presented to illustrate the advantages of these new algorithms.
\end{abstract}

\begin{keyword}
adaption \sep automatic \sep compuational complexity \sep function approximation \sep function recovery \sep minimization \sep optimization
...
The computational cost of Algorithm $M$, which is analyzed in Section~\ref{subsec:optcost}, is no more than the computational cost of Algorithm $A$.  However, it is observed that this cost can be substantially less than that of Algorithm $A$ if there is a large portion of $[a,b]$ where $f$ significantly exceeds its minimum value.

\item A lower bound on the computational complexity of problem~\eqref{appxprob} is derived in Section~\ref{subsec:appxcomp}.  This lower bound in Theorem~\ref{thm:A_cost} is proportional to $\sqrt{\norm[\frac12]{f''}/\abstol}$, which makes our algorithm essentially asymptotically optimal.  A lower bound for the computational complexity of  \eqref{optprob} has not yet been found, and Section~\ref{subsec:optcomp} explains the challenge in doing so.
...
\chL(x) = \min \biggl\{ l \in \mathbb{N}_0 :  \biggl\{ \biggl[\frac 18 \fC\left(3\cdot2^{-l(x)}h_0\right) + \frac 32 \biggr]   \norm[\bar{I}_{x,l}]{f''} \\
+ \frac18\norm[{I_{x^*,l}}]{f''} \biggr\} \Bigl(2^{-l}h_0\Bigr)^2 
+  3 \abs{f'(x)} \Bigl( 2^{-l}h_0 \Bigr) \\
\le f(x) - f(x^*)   \biggr\}.
\end{multline}
...
\begin{multline*}
\frac 18 B_{j,s}(f,\cp)(x_j-x_{j-1})^2 + \hM -\abstol - \min(f(x_{j-1}),f(x_{j})) \\
...
If $f(x)$ is close to the minimum function value, $f(x^*)$,  for $x$ in much of $[a,b]$, then $\chL(x)$ may be quite large or $L(x)$ may determine the computational cost of Algorithm $M$.  In this case, the computational cost for minimization may be similar to that for function approximation.  However, if $f(x)$ is significantly larger than $f(x^*)$, for $x$ in a large part of $[a,b]$, then $\chL(x)$ may have a quite moderate value, independent of the error tolerance.  In this case, the computational cost for minimization may significantly smaller than that for function approximation. 

\subsection{Lower Complexity Bound} \label{subsec:optcomp}  The optimization problem \eqref{optprob}  for  functions in the whole Sobolev space $\cw^{2,\infty}$ has a similar lower complexity bound as \eqref{lowbdW} for the function approximation problem by a similar proof.  However, for  functions only in the cone $\cc$, the lower complexity bound proof for the function approximation does not have a direct analog for the optimization problem.  The bump, $\tgamma f_3$, subtracted from $f_0: x \mapsto x^2/2$ may not affect the minimum function value, depending on the location of the bump's center.  We  have not yet derived a lower bound on the complexity of the optimization problem \eqref{optprob} for functions in  $\cc$.  
...
\includegraphics[width=65mm]{figure/traub_funappxNoPenalty_g_test.eps}
  \\ a)  & b)
  \end{tabular}
...
{\footnotesize
\setlength{\tabcolsep}{.18em} % set table width
...
{\footnotesize
\setlength{\tabcolsep}{.11em} % set table width
\begin{tabular}{|c|rrr|rrr|rrrrrr|rr|}
\hline
    Test      &     \multicolumn{3}{c|}{Mean Number of Points} & \multicolumn{3}{c|}{Mean Time Used}  & \multicolumn{6}{|c|}{Success (\%)} &  \multicolumn{2}{c|}{Failure (\%)}
