...
real-valued functions on the finite interval $[a,b]$, we will  construct
...
\end{gather}
Here $A$ and $M$ will depend only on function values. They will choose the data sites in $[a,b]$ adaptively, depending on the
function values already obtained at the data sites chosen previously.  The algorithms will sample more densely in places where it is required.  These
algorithms will also automatically determine when to stop sampling the input function
...
\]
Moreover, $f''([\alpha, \beta])$ is defined as the interval containing all values of $f''(x)$ for $x \in [\alpha,\beta]$, where $[\alpha,\beta] \subseteq [a,b]$.  Furthermore, $\abs{f''([\alpha, \beta])}$ is the interval containing all values of $\abs{f''(x)}$ for $x \in [\alpha,\beta]$.  We define a
measure of how small $\abs{f''(x)}$ can be for  $x \in [\alpha, \beta]$ as follows:
\begin{equation} \label{minfppdef}
m(f,[\alpha, \beta]) = \min \abs{f''([\alpha, \beta])}.
\end{equation}
For any intervals $I , J \subseteq [a,b]$, it follows from the definition~\eqref{minfppdef}
that
...
$\max \abs{f''_2(x)} \le 20$ for all $x \in [-1,1]$, so $f_2 \in
\cc$ if $\fC(0) \ge 2$.
...
\nonumber
 & \le \max\bigl(\fC(x_{i-1}-x_{i-3})m(f,[x_{i-3},_{i-1}]),\fC(x_{i+2}-x_i)m(f,[x_i,x_{i+2}])\bigr) \\
\nonumber & \le  \max\bigl(\fC(x_{i-1}-x_{i-3})\abs{f[x_{i-3},x_{i-2},x_{i-1}])},\bigr.  \\
\nonumber &\qquad  \qquad \bigl. \fC(x_{i+2}-x_i)\abs{f[x_i,x_{i+1},x_{i+2}]}\bigr) \\
& = \max\bigl(B_-(f,[x_{i-1},x_i]),B_+(f,[x_{i-1},x_i])\bigr)=: B(f,[x_{i-1},x_i]).
\end{align}
...
& =: B(f,[x_{i-1},x_i]),
\end{align}
where $B_+(f,[x_{i-1},x_i])=B(f,[x_{i-1},x_i])$ and $ B_-(f,[x_{i-1},x_i])=0$.
...
%\item[Step 1.] \label{stage1} Compute $f[x_{i-1}, x_{i}, x_{i+1}]$ for those values of $i = \{1,2,\ldots,n-1\}$ as in \eqref{divdiff}.
...
\item \label{stage2}  If $$\frac{(x_i - x_{i-1})^2B_+(f,[x_{i-1},x_i])}{8} > \abstol,$$
split in half those intervals $[x_i,x_i-1]$ for which $i, i+1$, or $i+2$ lies in $\mathcal{I}$ where the length of these intervals
are the largest.
If $$\frac{(x_i - x_{i-1})^2B_-(f,[x_{i-1},x_i])}{8} > \abstol,$$
split in half those intervals $[x_i,x_i-1]$ for which $i-2, i-1$, or $i$ lies in $\mathcal{I}$ where the length of these intervals
are the largest. %Suppose
