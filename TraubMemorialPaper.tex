...
The definition of $L(x)$ in \eqref{eqn:defoflx} is implicit because $l$ in the definition appears in the norm $\norm[\bar{I}_{x,l}]{f''}$.  We may derive a more explicit upper bound on the computational cost of $A$ by noting that 
\begin{equation*}
\norm[\bar{I}_{x,l}]{f''}
\end{equation*}

upper bound in theorem \ref{thm:cost} provides a 

From Theorem~\ref{thm:cost}, we know for very small $\abstol$,
...
\begin{align*}
\cost(S,f,\abstol)  & \lesssim \frac{n_0}{(b-a)}\int_a^b \sqrt{\frac{\fC\left(0\right)  h_0^2 |f''(x)|}{8\abstol}} \, \dif x +1 \\
