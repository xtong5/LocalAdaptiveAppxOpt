...
\end{multline}
The error of this approximation is bounded as follows from \cite{??}
...
In particular, $(x_j-x_{j-1})^2 \le (2^{-L(x)}h_0)^2$, which is part of the proof of \eqref{Blenorm}.  Also, note that
  \begin{align*}
  B_{j,+}(f,\cp)
  & \le \fC(x_{j+2}-x_{j-1})f[x_{j},x_{j+1},x_{j+2}] \qquad \text{by}~\eqref{bpf} \\
...
We now show that $\tell(x) \le \tL(x)$ by a similar argument to that used to derive an upper bound on the computational cost of Algorithm $A$.  Fix $x \in [a,b]$.  If $\tell(x) > \tL(x)$, then at some iteration $I_{x,\tL(x)}$ must be split in Step~\ref{stagemin2} of $M$.  Let $\cp = \{x_j\}_{j=0}^n$ denote the partition of $[a,b]$ at this iteration, and then identify the closure of $I_{x,\tL(x)}$ as $[x_{i-1},x_i]$ for some $i$, where $x_i-x_{i-1}=2^{-\tL(x)}h_0$. 


\textbf{start fixing here}


Referring to Step~\ref{stage2} of $M$, it is necessary that $i \in \widetilde{\mathcal{I}}$ for $I_{x,\tL(x)}$ to be split.  We show that this is impossible.

First we bound $\min(f_{x_{j-1}},x_j) - \widehat{M}$, $j=i-2, \ldots, i+2$, which must be non-negative by definition, and which appears in the definitions of $\ci_{\pm}$ and $\ci_{\pm k}$ in Algorithm $M$.  
Let $[x_{i^*-1}, x^*_i[$ be the interval in $\cp$ containing the location of the minimum, $x^*$.  Note that by \eqref{appxerrbdapc} it follows that
\[
