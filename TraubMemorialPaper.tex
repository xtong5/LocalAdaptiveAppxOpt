...
\begin{equation}
\norm{f''} \le \bignorm[1]{\sqrt{\abs{f''}}}^2 \le (b-a)\Bignorm[1]{\sqrt{\abs{f''}}}^2.
\end{equation}
...
& \le \sup_{a \le x \le b} \min\bigl\{\tB(f,x,h,h) : h \in [0,\fh], \ a \le x - h < x+h \le b \bigr \} \qquad \text{by } \eqref{conedef} \\
& \le \sup_{a \le x \le b} \min\bigl\{\fC(h) : \max(h_\pm) \in [0,\fh] \bigr \} \qquad \text{by } \eqref{conedef} \\
\end{align*}
