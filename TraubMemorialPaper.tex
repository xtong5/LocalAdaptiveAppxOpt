...
We cannot \scnote{missing a verb} of $f''(x)$ for a specific $x$ based only on values of $f$. However,
...
\begin{gather} 
\nonumber
f[x_1] = f(x_1), \quad f[x_1, x_2] = \frac{f[x_2] - f[x_1]}{x_2-x_1},  \quad
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Function Approximation Algorithm, $A$\scnote{A or S. Here, above, and below}} \label{sec:fappx}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{algo}
For some finite interval, $[a,b]$, some fixed $\fh >0$, and some non-decreasing
$\fC:[0,b-a] \to [1, \infty)$, let $f:[a,b] \to \reals$ and $\varepsilon >0$ \scnote{$\abstol$?} be
...
\item[Step 1.] \label{stage1} Check for convergence. 
Let   \scnote{$\abstol$? below}
\[
...
\item[Step 2.] \label{stage2}  Split in half those intervals $[x_i,x_i-1]$ such that $i-1, i$, or $i+1$ lies in $\mathcal{I}$. %Suppose
