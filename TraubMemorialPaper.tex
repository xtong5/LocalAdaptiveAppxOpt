...
We now show that $\tell(x) \le \tL(x)$ by a similar argument to that used to derive an upper bound on the computational cost of Algorithm $A$.  Fix $x \in [a,b]$.  If $\tell(x) > \tL(x)$, then at some iteration $I_{x,\tL(x)}$ must be split in Step~\ref{stagemin2} of $M$.  Let $\cp = \{x_j\}_{j=0}^n$ denote the partition of $[a,b]$ when $I_{x,\tL(x)}$ is to be split, and then identify the closure of $I_{x,\tL(x)}$ as $[x_{i-1},x_i]$ for some $i$, where $x_i-x_{i-1}=2^{-\tL(x)}h_0$. We show that such a scenario is impossible, thus contradicting the assumption that $\tell(x) > \tL(x)$.
...
\min\bigl(f(x_{j-1}),f(x_j) \bigr) \ge f(x) - \abs{f'(x)}3 \cdot 2^{-\tL(x)}h_0 - \frac{\norm[\bar{I}_{x,\tL(x)}]{f''}\bigl(3 \cdot 2^{-\tL(x)}h_0\bigr)^2}{6}
\end{multline}
