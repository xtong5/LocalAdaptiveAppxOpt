...
Moreover, non-adaptive algorithms are unable to solve \eqref{appxprob}  or \eqref{optprob} using a finite number of function values if the set of interesting functions, $\cc$, is a cone, unless these non-adpative algorithms solve these problems exactly.  Suppose that a successful non-adaptive algorithm for error tolerance $\abstol$ is given the zero input function and samples on some partition $x_{1:n}$.  Construct a function $f^*$ with nonzero minimum value that vanishes on this partition.  The error of the non-adaptive algorithm for $f^*$ must then be some positive $\delta$.  Since $f^* \in \cc$, so is $cf^*$ for any positive $c$, and so the non-adaptive algorithm must have an error of $c\delta$ for some element of $\cc$, where $c\delta$ may be made arbitrarily large.  This is a contradiction.
