...
\subsection{Key Ideas in Our Algorithm}
...
\mathcal{I}_{\pm} = \{1,2,\ldots,n-1,n\}.$$
...
Compute $B_{i,\pm}(f,\cp)$ as in~\eqref{bpdef} for all $i \in \mathcal{I}_{\pm}$.
Let
\[
\mathcal{I}_\pm = \left\{i \in \mathcal{I}_{\pm}: \frac{(x_i - x_{i-1})^2B_{i,\pm}(f,\cp)}{8}  > \abstol \right\}.
\]
 If $\mathcal{I}_+ \cup \mathcal{I}_-=
...
Let
\begin{gather*}
\widehat{\mathcal{I}}_{\pm} = \biggl\{i \in \{1,2,\ldots,n\}: i\mp k \in \ci_{\pm}, \ k \in \{0,1\}   \biggr\}, \quad  \\
...
$\widetilde{\mathcal{I}}$. Update $n$, $\cp = \{x_j\}_{j=0}^n$, $\mathcal{I}_{\pm}$,
and the $f[x_{j-1}, x_{j}, x_{j+1}]$ accordingly. The updated $\ci_{\pm}$ contains
indices corresponding to the old subintervals or halves of old subintervals
whose indices were in the old $\ci_{\pm}$. Return to Step~\ref{stage1}.
\end{enumerate}
\end{algoA}
...
$-$, and for some $k \in \{0,1\}$, $j = i - sk \in \ci_s$. The crux of the
proof is to demonstrate that
\begin{equation} \label{Blenorm}
B_{j,s}(f,\cp)(x_j-x_{j-1})^2 \le  \fC\left(3h\right) h^2  \norm[\bar{I}_{x,L(x)}]{f''}.
\end{equation}
We give the proof for  $s =+$.  The proof for $s= -$ is similar.  Since $j \in \ci_+$, it follows that $j + k \in \widehat{\ci}_+$ for all $k \in \{0,1\}$. Since $i \in \widetilde{\ci}$ and $j \in \ci_+$, it also follows that
\begin{equation} \label{conditionone}
h = x_{i} - x_{i-1} = x_{j+k} - x_{j+k-1} \qquad \text{for all } k \in \{0,1,2\}.
\end{equation}
In particular, $(x_j-x_{j-1})^2 =h^2$, which is part of the proof of
...
From the definition of $L(x)$ in~\eqref{eqn:defoflx}, we know
\[\frac{2^{L(x)}}{h_0} \ge \sqrt{\frac{\fC\left(3\cdot 2^{-L(x)} h_0\right)\norm[\bar{I}_{x,L(x)}]{f''} }{8\abstol}}\]
and
\[\frac{2^{L(x)-1}}{h_0} \ge \sqrt{\frac{\fC\left(3\cdot 2^{-L(x)+1} h_0\right)\norm[\bar{I}_{x,L(x)-1}]{f''} }{8\abstol}}\].
Thus, we have
\begin{align*}
\cost(A,f,\abstol) & \le \frac{1}{h_0}\int_a^b 2^{L(x)} \, \dif x +1 , \\
& < \int_a^b 2 \sqrt{\frac{\fC\left(6 \cdot 2^{-L(x)} h_0\right)\norm[\bar{I}_{x,L(x)-1}]{f''} }{8\abstol}}\, \dif x +1\\
& = \int_a^b  \sqrt{\frac{\fC\left(6 \cdot 2^{-L(x)} h_0\right)\norm[\bar{I}_{x,L(x)-1}]{f''} }{2\abstol}}\, \dif x +1
\end{align*}

For small $\abstol$,
$L(x)$ becomes large, $\norm[\bar{I}_{x,L(x)-1}]{f''}$ approaches $|f''(x)|$, and
$3\cdot 2^{-L(x)}h_0$ tends to $0$. Thus, for small $\abstol$, the upper bound on computational cost becomes
\begin{align}
\nonumber
\cost(A,f,\abstol)  &\lesssim \int_a^b \sqrt{\frac{\fC\left(0\right)   |f''(x)|}{2\abstol}} \, \dif x +1 \\
