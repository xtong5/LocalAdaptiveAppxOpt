...
I_{x_{i-1},\ell(x_{i-1})} = [x_{i-1},x_{i}]$ for $i \in 1\!:\! n$. Furthermore
...
\begin{align*}
\norm[{[\alpha,\beta]}]{f''}
& \le  \lim_{\delta \downarrow 0}\min\bigl\{\tB(f,\alpha,\beta,h,h) : h \in (\beta - \alpha,\fh) \bigr \} \quad \text{by}~\eqref{conedef} \\
& \le  \min\bigl\{\fC(h) \max\bigl(\norm[-\infty,{[\beta - h, \alpha]}]{f''},\norm[-\infty,{[\beta, \alpha+h]}]{f''}\bigr) : h \in (\beta - \alpha,\fh) \bigr \} \\
& \le \min_{\beta - \alpha < h < \fh} \frac{\fC(h)}{(h-\beta+\alpha)^2}\norm[\frac 12,{[\alpha,\beta]}]{f''} \qquad \text{by}~\eqref{onebdm}.
\end{align*}
\end{proof}
