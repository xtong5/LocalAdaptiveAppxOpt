...
$h_{\pm}$ satisfying the conditions in the definition of $\cc$ in
\eqref{conedef}, it follows that
\begin{multline*}
...
\centering
\begin{tabular}{cc}
\includegraphics[width=5.7cm]{figure/f3ppplot.eps} \quad
...
second derivative.  (b) The non-uniform sampling density of Algorithm $A$ for input function
$-f_1$ defined by $\delta = 0.3$ and $c = -0.2$. A total of~$4$ iterations
and~$59$ points are used to meet the error tolerance of~$0.01$. Both of these plots are
...
Figure \ref{f3fig}(b) displays the function $-f_1$ defined in
...
\begin{proof}
	...
	n+1$. Figure \ref{f3foolplot}(a) illustrates this situation. Since $0$ and $\pm
...
\begin{tabular}{cc}	
		...
		\\ (a) & (b)
		\end{tabular}
		\caption{(a) The fooling functions $\pm f_1$ used to prove \eqref{lowbdW}. The case
		$n=15$ is shown.  (b) The same situation as in Figure~\ref{f3fig}(b), but now
with Algorithm $M$. Again~$4$ iterations are used, but only~$31$ non-uniform
sampling points are needed to obtain the minimum of $-f_1$.  This figure is reproducible by \texttt{TraubMemorial.m}
		in GAIL.}
		\label{f3foolplot}
\end{figure}
...
\ref{f3fig}(b), but this time with the sampling points used for
