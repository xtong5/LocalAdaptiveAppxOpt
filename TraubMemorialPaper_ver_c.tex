% A list of all figure files produced by MATLAB: yuhan, fred
...
\begin{figure}
	...
	\caption{A plot of $\abs{f''(x)}$ (solid) for some sample $f$.  If $f \in \cc$, then
	$\norm[{\alpha,\beta}]{f''}$ (dashed) \label{fig:ConeDef}} is bounded above in
    terms of  $\norm[-\infty,{[\beta - h_-,\alpha]}]{f''}$ and $\norm[-\infty,{[\beta,
    \alpha+h_+]}]{f''}$ (dashed) multiplied by inflation factors $\fC(h_{\pm})$.  Moreover,
    \textbf{Fred needs to add more here}
\end{figure}
...
simulations for each test function:
%\funappxg{} time $/$ \funappxglobalg{} time (blue),
%\funappxg{} \# of samples $/$ \funappxglobalg{} \# of samples (orange),
...
%our old globally adaptive algorithm \texttt{funappxglobal\_g},
and Chebfun are
...
and override the default tolerance to $10^{-6}$. The
...
rates of \funappxg (local) %, \funappxglobalg (global),
...
{\footnotesize
\setlength{\tabcolsep}{1.2em} % set table width
		\begin{tabular}{crrrrrrrrrrrr}	
		 \hline	
			     &    \multicolumn{2}{c}{\bf Mean Points}   & \multicolumn{2}{c}{\bf Mean Time Used}  & \multicolumn{2}{c}{\bf Success (\%)}
	  \\ \hline  functions &  \multicolumn{1}{r}{local}  &    \multicolumn{1}{r}{Chebfun}  &   \multicolumn{1}{r}{local}   &  \multicolumn{1}{r}{Chebfun} &    \multicolumn{1}{r}{local}  &   \multicolumn{1}{r}{Chebfun}
\\ \toprule
          $f_1$   &   5659       &   116    &   0.0084         &  0.0403 &    100        &  0
\\        $g_1$   &    3690       &   43    &   0.0091          &   0.0104 &    100       &  3
\\        $g_2$   &   11835      &    22    &   0.0114          &   0.0061 &    100        &  3  	
\\ \hline
\end{tabular}
%
...
\begin{tabular}{ccrccrccrccrccrccrccrccrccrc}		
		...
		% \\
			 functions &  \multicolumn{1}{c}{\funming} &  \multicolumn{1}{c}{\fminbnd}  &  \multicolumn{1}{c}{\texttt{min}}
		  &  \multicolumn{1}{c}{\funming}  &  \multicolumn{1}{c}{\fminbnd }  &  \multicolumn{1}{c}{\texttt{min} }  &  \multicolumn{1}{c}{\funming} & \multicolumn{1}{c}{\fminbnd} & \multicolumn{1}{c}{\texttt{min}}
			\\ \toprule
...
\funappxg{} obtained the correct answer in all cases,
even for $g_1$, which is outside the cone $\cc$. %The new locally adapative
%algorithm uses fewer samples than the older globally adaptive algorithm.
Because it is a higher order algorithm, Chebfun generally used substantially fewer number of samples than
\funappxg, but Chebfun's run time is longer than \funappxg for a
significant proportion of the cases; see Figure~\ref{f3chebfig}(b). However, Chebfun rarely
