...
The idea of Algorithm $A$ is to use divided differences to provide lower bounds on $\norm[{[\beta - h, \alpha]}]{f}$ and $\norm[{[\beta, \alpha+h]}]{f}$ via \eqref{NDDbdm}, which then provide an upper bound on $\norm[{[\alpha,\beta]}]{f}$ via the definition of the cone, $\cc$, in \eqref{conedef}.  This in turn yields an upper bound on the spline error via \eqref{appxerrbda}.  After stating the algorithm, we prove its effectiveness.
