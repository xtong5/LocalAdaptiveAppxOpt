...
and a  number $\fC_0 \ge 1$. Let \scnote{In code \funappxg, $\fh$'s denominator is $\ninit-2$}
\begin{equation}
\label{hCdef}
...
\begin{figure}[t]
	...
	\caption{(a) The nonuniform sampling density of Algorithm $A$ for input function
		...
		and~$59$ points and $4$ iterations are used to meet the error tolerance 
		of~$0.02$.
		(b) The same situation as in (a), but now
		with Algorithm $M$. Again~$4$ iterations are used, but only~$27$ nonuniform
		sampling points and $3$ iterations are needed to obtain the minimum of $-f_1$. 	
		\label{f3foolplot}}
\end{figure}
...
\frednote{For which test functions?  $f_1, f_2,$ and $f_3$ all together?}\scnote{all}
...
1000$ \frednote{We need $n_{\ninit}$ here instead}\scnote{216}, $\fC (h) =
3 \fh/(\fh-h)$\scnote{constant is 10 instead of 3}, and $\abstol = 10^{-6}$. Our new algorithm \texttt{funappx\_g}
and Chebfun are
...
{\footnotesize
\setlength{\tabcolsep}{0.38em} % set table width
...
long service to our research community.
