...
\tB(f,x,h_-,h_+)=\\
\begin{cases}
  \max\bigl(\fC(h_{-}) m(f,\alpha-h_-,\alpha),\fC(h_{+}) m(f,\beta,\beta+ h_+)\bigr), & a + h_- \le \alpha  \le \beta \le b-h_+,\\
\fC(h_{-}) m(f,\alpha-h_-,\alpha), & a + h_- \le \alpha  \le b-h_+ <\beta,\\
\fC(h_{+}) m(f,\beta,\beta+ h_+), & \alpha  < a + h_- \le \beta \le b-h_+.
\end{cases} %\quad
 %\Vastr
\end{multline*}
...
x_{i+1} - x_{i-1} \le \fh, \qquad i = 1, \ldots, n-1.
\end{equation}
For the interval $ [x_{i-1},x_i]$,  denote
\begin{align*}
&h_- = x_{i-1} - x_{i-3}, \qquad i=3,4,\ldots,n,\\
 &h_+ = x_{i+2} - x_i, \qquad i=1,2,\ldots,n-2.
\end{align*}
...
2\fC(x_{i+2}-x_{i})\abs{f[x_i,x_{i+1},x_{i+2}]},  & i=1,\ldots,n-2,
\\ 0, & i=n-1,n.
\end{cases} \\
\label{bmf}
B_{i,-}(f,\cp)&: =\begin{cases}
0,  & i=1,2,
\\ 2\fC(x_{i-1}-x_{i-3})\abs{f[x_{i-3},x_{i-2},x_{i-1}])}, & i=3,\ldots,n,
\end{cases} \\
\nonumber
...
\begin{equation}
\label{nodefinition}
 n_{\ninit} \ge 5  \qquad \fh := \frac{2(b-a)}{n_{\ninit}-2}.
\end{equation}
...
L(x) = \min \left\{ l \in \mathbb{N}_0 :  \frac{1}{8} \fC\left(2\cdot2^{-l}h_0\right)(2^{-l}h_0)^2\norm[\bar{I}_{x,l}]{f''} \le \abstol \right\}.
\end{equation}
...
B_{j,s}(f,\cp)(x_j-x_{j-1})^2 \le  \fC\left(2h\right) h^2  \norm[\bar{I}_{x,L(x)}]{f''}.
\end{equation}
...
\begin{equation*}
2^{L(x)} = \min\left\{2^l:  2^l \ge \sqrt{\frac{\fC\left(2\cdot 2^{-l} h_0\right) h_0^2 \norm[\bar{I}_{x,l}]{f''} }{8 \abstol}}, \  l \in  \natzero\right\}.
\end{equation*}
\end{theorem}
...
$2\cdot 2^{-L(x)}h_0$ tends to $0$. Thus, for small $\abstol$, we have
