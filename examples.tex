...
for $i=1,2, \mbox{and } 3$. Figure~\ref{fig:testfunction} shows the shapes of
...
\begin{figure}[th]
\centering
  \includegraphics[width=65mm]{figure/traub_funappxNoPenalty_g_testfun.eps}
  \caption{Test functions for comparing \texttt{funappx\_g}, \texttt{funappxglobal\_g}, and Chebfun.}  
...
\hline
\hline
     $f$ &     \multicolumn{3}{c|}{Number of Points}  & \multicolumn{3}{c|}{Time Used}  & \multicolumn{3}{c|}{Success}  &  Failure
\\   &  Local  &  Global    &  Chebfun & Local       &  Global  & Chebfun    & Local   &  Global  & Chebfun  &   Chebfun  
\\ \hline
...
\funappxglobalg{} achieved 100\% success rates. The former worke better than
\funappxglobalg{} for all test cases in terms of average number of sampling points.
It is also more \emph{cautionary} as it issued more warnings than \funappxglobalg.
%
We can see that Chebfun are particularly challenged by the highly oscillatory
functions $g_1$ and $g_2$ in terms of run time and success rates. In some
instances of the peaky functions $g_3$ and $g_6$, it failed to approximate them
satisfactorily for about 10\% of the cases.
